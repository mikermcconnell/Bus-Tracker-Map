// Platform Map Logic (Ancient JS / Safe Mode)
...
(function () {
    ...
    try {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
                        ...
                        log('XHR Error: ' + xhr.status, 'ERROR');
                        callback([]);
                    }
                }
            };
        ...
        function updateMap(vehicles) {
            ...
            log('Updated ' + vehicles.length + ' buses', 'SUCCESS');
        }

        function loop() {
            fetchVehicles(function (vehicles) {
                updateMap(vehicles);
                setTimeout(loop, 10000); // Recursive timeout
            });
        }

        // Start
        log('Starting loop...', 'INFO');
        loop();

        // Visual Indicator
        var h1 = document.createElement('h1');
        h1.textContent = 'JS Alive (XHR Mode)';
        ...
        document.body.style.backgroundColor = 'red'; // Visual crash indicator
    }
})();
