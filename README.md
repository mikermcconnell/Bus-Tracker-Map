...
- The monitor checks the production Allandale departure-board API during service. When at least two upcoming Barrie departures remain without any exact live trip match for 60 minutes, it sends one `ALLANDALE_LIVE_DATA_UNAVAILABLE` email to `ALERT_RECIPIENT`. It sends no reminders and closes the incident with `ALLANDALE_LIVE_DATA_RECOVERED` after two consecutive healthy checks. Configure this with `ALLANDALE_DEPARTURES_URL`, `ALLANDALE_LIVE_ALERT_AFTER_MIN`, `ALLANDALE_LIVE_MIN_DEPARTURES`, and `ALLANDALE_LIVE_RECOVERY_CHECKS`.
